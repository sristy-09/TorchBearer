...
export default function ProfilePostCard({ post }: Props) {
  ...
  const handlePostClick = () => {
    ...
    if (!space || !topic) return;

    if (currentUser?.role === "admin") {
      navigate(
        `/space/${space._id}/topic/${topic._id}/posts#post-${post._id}`
      );
    } else {
      checkSpaceAccessAndNavigate(space._id, topic._id);
    }
  };
  ...
  const checkSpaceAccessAndNavigate = async (
    ...
    try {
      ...
      if (isMember) {
        navigate(
          `/space/${spaceId}/topic/${topicId}/posts#post-${post._id}`
        );
      } else {
  ...
  return (
    <>
      ...
      {/* ACCESS DIALOG */}
      <AlertDialog
        open={spaceAccessDialogOpen}
        onOpenChange={setSpaceAccessDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base">
              ...
              .<br />
              <br />
              To view this post, you need access to this space.
            </AlertDialogDescription>
          </AlertDialogHeader>
